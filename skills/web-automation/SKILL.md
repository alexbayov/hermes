...
browser:
  # NOTE: headless and viewport are currently set via function arguments/CLI,
  # NOT read from YAML. Only trace is honored by the engine.
  trace: bool             # default false — enables Playwright trace on failure
capabilities:
  redaction:
    # NOTE: redaction is controlled by secret: true on each field in fields_schema.
    # The 'enabled' flag is decorative — it does not enable/disable redaction.
    enabled: bool
...
- Mark all sensitive fields with `secret: true` in `fields_schema`. **This is what controls redaction** — not `capabilities.redaction.enabled` (that flag is decorative and does not affect behavior).
...
- `strategy` is a **reserved field** — only `max_attempts` currently affects retry behavior.
